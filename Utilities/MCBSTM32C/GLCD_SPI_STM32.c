...
void GLCD_ClearRegion (unsigned short color, unsigned int x, unsigned int y, unsigned int cw, unsigned int ch) 
{
	unsigned int i;
...
*                   ch:       character height in pixels                       *
*                   c:        pointer to character bitmap                      *
*                   BackColor: color of the text background                    *
*                   TextColor: color of the text                               *
*   Return:                                                                    *
*******************************************************************************/

void GLCD_DrawCharWithColor (unsigned int x, unsigned int y, unsigned int cw, unsigned int ch, unsigned char *c, unsigned short BackColor, unsigned short TextColor) {
	unsigned int i, j, k, pixs;
	...
	Color[TXT_COLOR] = TextColor;
	Color[BG_COLOR] = BackColor;
...
void GLCD_DisplayCharWithColor (unsigned int ln, unsigned int col, unsigned char fi, unsigned char c, unsigned short BackColor, unsigned short TextColor) {

	c -= 32;
	switch (fi) {
	case 0:  /* Font 6 x 8 */
		GLCD_DrawCharWithColor(col *  6, ln *  8,  6,  8, (unsigned char *)&Font_6x8_h  [c * 8], BackColor, TextColor);
		break;
	case 1:  /* Font 16 x 24 */
		GLCD_DrawCharWithColor(col * 16, ln * 24, 16, 24, (unsigned char *)&Font_16x24_h[c * 24], BackColor, TextColor);
		break;
	case 2:  /* Font 20 x 40 */
		GLCD_DrawCharWithColor(col * 20, ln * 40, 20, 40, (unsigned char *)&Font_20x40_h[c * 120], BackColor, TextColor);
		break;
	}
}
...
*   Parameter:      ln:        line number                                     *
*                   col:       column number                                   *
*                   fi:        font index (0 = 6x8, 1 = 16x24, 2= 20*40)       *
*                   s:         pointer to string                               *
*                   BackColor: color of the text background                    *
*                   TextColor: color of the text                               *
*   Return:                                                                    *
...
void GLCD_DisplayStringWithColor (unsigned int ln, unsigned int col, unsigned char fi, unsigned char *s, unsigned short BackColor, unsigned short TextColor) {

	while (*s) {
		GLCD_DisplayCharWithColor(ln, col++, fi, *s++, BackColor, TextColor);
	}
}
...
*   Return:                                                                    *
*******************************************************************************/
void GLCD_BargraphColor (unsigned int x, unsigned int y, unsigned int w, unsigned int h, unsigned int val, unsigned short frontC, unsigned short backC) {
	int i,j;

	Color[BG_COLOR]= backC;
	Color[TXT_COLOR]= frontC;
...
*   Return:                                                                    *
*******************************************************************************/
void GLCD_BargraphWithCorlorLineX (unsigned int x, unsigned int y, unsigned int w, unsigned int h, unsigned int val/*, unsigned short BackColor, unsigned short TextColor*/) 
{
	int i,j;
  ...
  for (i = 0; i < h; i++) 
  {
    for (j = 0; j <= w-1; j++) 
	{
      if(j >= val) 						     //no value yet
         wr_dat_only(WHITE);		     //bgcolor	  ori Black
	  else if( j< (w/2) )		             //value
         wr_dat_only(GREEN);
	  else if( j>= (w/2) && j<= (3*w/4) )
		 wr_dat_only(YELLOW);
	  else if( j>= (3*w/4) && j<= w )
		 wr_dat_only(RED);
    }
...
void GLCD_BargraphWithCorlorLineY (unsigned int x, unsigned int y, unsigned int w, unsigned int h, unsigned int val/*, unsigned short BackColor, unsigned short TextColor*/) 
{
	int i,j,itemp;


	val = (val * h) >> 10;                     //Scale value 
	...
	for (i = 0; i < h; i++) 
	{
		...
		for (j = 0; j <w; j++) 
		{
			if(itemp >= val) 						 //no value yet
				wr_dat_only(BLACK);		         //bgcolor		  (ori BLACK)
			else if( itemp< (h/3) )		         //value
				wr_dat_only(GREEN);
			else if( itemp>= (h/3) && itemp<= (2*h/3) )
				wr_dat_only(YELLOW);
			else if( itemp>= (2*h/3) && itemp<= h )
				wr_dat_only(RED);
		}
	}
	wr_dat_stop();
...
void GLCD_Bitmap         (unsigned int x,  unsigned int y, unsigned int w, unsigned int h, unsigned char *bitmap) {	//char bitmap
	int i, j;
	//  unsigned short *bitmap_ptr = (unsigned short *)bitmap;
	unsigned char *bitmap_ptr = bitmap;

	GLCD_SetWindow (x, y, w, h);

	wr_cmd(0x22);
	wr_dat_start();
	/*  for (i = (h-1)*w; i > -1; i -= w) {					  //scan from last line up to top
	for (j = 0; j < w; j++) {
	wr_dat_only (bitmap_ptr[i+j]);
	}
	}	   */
	for (i = 0; i < (h-1)*w; i += w) 					     //scan from top to bot
	{
		for (j = 0; j < w; j++)
			wr_dat_only (bitmap_ptr[i+j]);
	}
