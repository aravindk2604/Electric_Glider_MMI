...
)
{
  ...
  return TM_SPI_Send(SPI_SDx, dat);
}
...
)
{
  ...
  TM_SPI_ReadMulti(SPI_SDx, buff, 0xFF, btr);
...
)
{
  ...
  TM_SPI_WriteMulti(SPI_SDx, (uint8_t *)buff, btx);
}
#endif
...
)
{
  ...
  do {
    d = xchg_spi(0xFF);
  } while (d != 0xFF && TIM_GetCounter(TIM2) ); /* Wait for card goes ready or timeout */
  if (d == 0xFF) {
...
)
{
  ...
  } while ((token == 0xFF) && 1);//TM_DELAY_Time2());
  if (token != 0xFE) {
...
//  
//  RCC_AHB1PeriphClockCmd(FATFS_CS_RCC, ENABLE);  // CHIP SELECT PIN, defined in defines.h GPIOB - pin 5
//  
...
//  
//  // GPIOB->BSRRL |= GPIO_Pin_5;  // PB5 set as high!  done within FATFS_CS_HIGH (see fatfs_sd.h)
//  
  ...
  if (send_cmd(CMD0, 0) == 1) {        /* Put the card SPI/Idle state */
    ...
    if (send_cmd(CMD8, 0x1AA) == 1) {  /* SDv2? */
      ...
      if (ocr[2] == 0x01 && ocr[3] == 0xAA) {        /* Is the card supports vcc of 2.7-3.6V? */
        while (send_cmd(ACMD41, 1UL << 30)&& 1);//TM_DELAY_Time2()  ) ;  /* Wait for end of initialization with ACMD41(HCS) */
        if (0 == send_cmd(CMD58, 0) && 1){ //TM_DELAY_Time2()  ) {    /* Check CCS bit in the OCR */
          for (n = 0; n < 4; n++) {
            ocr[n] = xchg_spi(0xFF);
          }
      ...
      while (send_cmd(cmd, 0) && 1);//TM_DELAY_Time2());      /* Wait for end of initialization */
      if (0 != send_cmd(CMD16, 512) || 1){// TM_DELAY_Time2()  ) {  /* Set block length: 512 */
        ty = 0;
      }
    }
  }
